...
export function DocumentPreview({ documentType, values }: DocumentPreviewProps) {
  return (
    <div
      id="document-preview"
      className="min-h-[29.7cm] rounded-lg border border-sage-200 bg-white p-10 font-serif text-slate-800 shadow-sm"
    >
      <header className="border-b border-sage-200 pb-4 text-center">
        ...
        {documentType.fields.map((field) => {
          ...
          return (
            <section key={field.key}>
              <h3 className="text-sm font-semibold tracking-wide text-sage-700 uppercase">
                {field.label}
              </h3>

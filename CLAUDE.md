...
- **Autenticação** (`app/auth.py`, `app/routers/auth.py`): senha com hash via `bcrypt` (não `passlib` — incompatibilidade conhecida entre `passlib` e `bcrypt>=4.1`), sessão via JWT (`python-jose`) armazenado em cookie `httponly` (`psyinsight_session`). Endpoints: `POST /api/auth/signup`, `/login`, `/logout`, `GET /api/auth/me`, `PUT /api/auth/me` (edição de perfil — nome, CRP, local de atendimento; atualização parcial, mesmo estilo de `saved_documents.update_saved_document`).
- **Modelo de usuário** (`app/models.py`): nome, e-mail, **CRP** (necessário para o cabeçalho dos documentos), **local de atendimento** (`local_atendimento`, opcional, editável via `PUT /api/auth/me` — ver "Bloco de assinatura automático" em "Design da IA"), senha com hash.
...
- **`PerfilPage`** (rota `/perfil`, link "Meu Perfil" no header do `DashboardPage`): formulário para editar nome, CRP e local de atendimento via `updateProfile` do `AuthContext`. Como `user` chega de forma assíncrona do `AuthContext`, os campos são inicializados vazios e resincronizados por um `useEffect` quando `user` muda; os inputs ficam desabilitados durante o salvamento (`isSaving`) para não perder edições em andamento caso a resposta do `PUT` resincronize o formulário no meio da digitação.
